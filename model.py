...
class Tacotron2(nn.Module):
    ...
    def forward(self, inputs):
        ...
        # bert_entention_embedding, bert_representation = self.emotion_distil_bert(input_ids, attention_mask=attention_mask)
        # bert_entention_embedding = bert_entention_embedding.detach()
        embedded_inputs = self.embedding(inputs).transpose(1, 2)
        embedded_text = self.encoder(embedded_inputs, input_lengths)
        embedded_speakers = self.speaker_embedding(speaker_ids)[:, None]
        len = int(embedded_text.size(1))
        embedded_speakers = embedded_speakers.repeat(1, len, 1)
        # broadcasted_bert_emotion_embedding = bert_entention_embedding.unsqueeze(1)
        # broadcasted_bert_emotion_embedding = broadcasted_bert_emotion_embedding.repeat(1, len, 1)
        encoder_outputs = torch.cat(
            (embedded_text, embedded_speakers)
        , dim=2)
